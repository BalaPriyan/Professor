...
API_ID = int(environ('API_ID', '12635510'))
API_HASH = environ('API_HASH', 'da59e4e56ec4fe35af603bd30208ecc5')
BOT_TOKEN = environ('BOT_TOKEN', '5829537823:AAHRdnev7hvNE92i1hfCi_GfXlrzmAoKNt0')
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001947006673').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('AUTH_CHANNEL', '-1001919160219')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://KING:KING@cluster0.uxw4hon.mongodb.net/?retryWrites=true&w=majority")
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1001659730523'))
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'FondnessMV_update')
